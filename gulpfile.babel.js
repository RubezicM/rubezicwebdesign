...
/// Get folder name
let dirArray = path.resolve(__dirname).split('\\');
let len = dirArray.length;
const folderName = dirArray[len - 1];


// check if in production or not
const PRODUCTION = yargs.argv.prod;

// host on htdocs localhost
const WEBSERVER = yargs.argv.host;
...
const paths = {
    ...
    vendors: {
        styles: 'src/assets/vendors/styles/*.css',
        scripts: 'src/assets/vendors/scripts/*.js',
    ...
    package: {
        src: ['.htaccess','./**/*','!.vscode','!node_modules{,/**}','!packaged{,/**}','!src{,/**}','!.babelrc','!.gitignore','!gulpfile.babel.js','!package.json','!package-lock.json'],
        dest: 'packaged'
    }
};
...
export const serve = (done) => {
    if(WEBSERVER){
        server.init({
            proxy: "http://localhost/" + folderName + "/"
        });
    } else {
        server.init({
            server: {
